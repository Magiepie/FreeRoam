...
if (!PlayerHandler.players[c.playerId].goodDistance(c.absX, c.absY, NPCHandler.npcs[i].absX, NPCHandler.npcs[i].absY, 15)) {
	...
	**/
		...
		if(c.trade11 > 0) {
			...
			c.sendMessage("@red@You can not pk scince you are to new so please wait another "+mins+" minutes.");
			resetPlayerAttack();
			return;
		}
			Client c2 = (Client) PlayerHandler.players[i];
			if (c.connectedFrom.equalsIgnoreCase(c2.connectedFrom)) {
 			c.sendMessage("Naw, no attacking same ip");
			resetPlayerAttack();
			return;
			}
	...
	public void activatePrayer(int i) {
		...
		if (i == 24 && c.playerLevel[1] < 65) {
			...
			c.sendMessage("You need a Defence level of 65 to use Chivalry.");
			return;
		}
		if (i == 25 && c.playerLevel[1] < 40) {
			c.getPA().sendFrame36(c.PRAYER_GLOW[i], 0);
			c.sendMessage("You need a Defence level of 40 to use Turmoil.");
			return;
		}
	...
	**/
	...
	public int calculateMeleeDefence()
    {
        ...
        if (c.prayerActive[0]) {
            ...
            defenceLevel += c.getLevelForXP(c.playerXP[c.playerDefence]) * 0.30;
        }
	...
	**/
	...
	public void handleDfs() {
		if (System.currentTimeMillis() - c.dfsDelay > 30000) {
			...
			c.sendMessage("The Dragonfire shield's power isn't done recharging.");
		}
	}
