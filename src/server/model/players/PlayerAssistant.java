...
public class PlayerAssistant{
	...
	public void writeCommandLog(String command)
	{
		...
		{				
			...
			bw.write("[" + c.playerName + "]: " + "[" + c.connectedFrom + "] "+  "::" + command);
			bw.newLine();
			bw.flush();
		} 
...
return -1;
	...
	public void openUpBank(){
		synchronized(c) {
			if(c.getOutStream() != null && c != null) {
				...
				c.flushOutStream();
				c.isBanking = true;
				c.sendMessage("@red@You can now use ::bankall");
			}
		}
	}
		...
		if(c.duelStatus <= 4 && !c.getPA().inPitsWait()) { // if we are not in a duel we must be in wildy so remove items
			if (!c.inPits && !c.inFightCaves()) {
				...
				if((c.playerRights == 2 && Config.ADMIN_DROP_ITEMS) || c.playerRights != 2) {
					...
					c.sendMessage("@red@You don't lose any items because you own this bitch");
					}
	...
	**/
	...
	public void removeObjects() {
		...
		objectToRemove(2524, 4780);
		objectToRemove(2523, 4777);
		objectToRemove(2526, 4775);
		objectToRemove(2526, 4791);
		objectToRemove(2527, 4791);
		objectToRemove(2529, 4791);
		objectToRemove(2530, 4791);
	}
	...
	public void addStarter() {
		if (!Connection.hasRecieved1stStarter(Server.playerHandler.players[c.playerId].connectedFrom)) {
			...
			c.getItems().addItem(995,10000000);
                	...
                	c.getItems().addItem(392,300);
			c.slayerTask = -1;
			c.sendMessage("@red@You can do ::train. Barrows brothers are on their hills");
	...
	public void checkPouch(int i) {
		if (i < 0)
			return;
		c.sendMessage("This pouch has " + c.pouches[i] + " essence in it.");		
	}
			...
			for (int i = 0; i < c.getItems().brokenBarrows.length; i++) {
				if (c.playerItems[j]-1 == c.getItems().brokenBarrows[i][1]) {					
					if (totalCost + 100000 > cashAmount) {
						breakOut = true;
						c.sendMessage("You need at least 100,000 gold to repair your Barrow items.");
						break;
					} else {
						totalCost += 100000;
					}
	...
	public void handleLoginText() {
		c.getPA().sendFrame126("Monsters", 13037);//ancient //
		c.getPA().sendFrame126("Minigames", 13047);
		c.getPA().sendFrame126("Bosses", 13055);
		c.getPA().sendFrame126("PvP Zones", 13063);
		c.getPA().sendFrame126("Skilling", 13071);
		c.getPA().sendFrame126("Random Teleports", 13081);
		c.getPA().sendFrame126("Monsters", 1300);//normal //
		c.getPA().sendFrame126("Minigames", 1325);
		c.getPA().sendFrame126("Bosses", 1350);
		c.getPA().sendFrame126("PvP Zones", 1382);
		c.getPA().sendFrame126("Random Teleports", 1454);
		c.getPA().sendFrame126("Skilling", 1415);
		c.getPA().sendFrame126("Donator Island", 13089); //ancient
		c.getPA().sendFrame126("Donator Island", 7457); //normal
		c.getPA().sendFrame126("Go to the Donator Island!", 7458); //normal
		c.getPA().sendFrame126("Go to the Donator Island!", 13090); //ancient
//New below, for quest tab.
		c.getPA().sendFrame126("Custom Quest Tab", 663); //FridderScape 474
		c.getPA().sendFrame126("@whi@Open bank", 7332);
		c.getPA().sendFrame126("@whi@Food", 7334);
		c.getPA().sendFrame126("@gre@Tele to slayer task!", 7335);
		c.getPA().sendFrame126("@whi@Pots", 7336);
		c.getPA().sendFrame126("@yel@Admin Commands", 7337);
		c.getPA().sendFrame126("@blu@Commands", 7341);
		c.getPA().sendFrame126("@whi@Mod Commands", 7342);
...
//end of new for quest tab, start of ~commands interface.~
                                        /*sendQuest("~~~~~~~~~~~~", 8147);
                                        sendQuest("@red@Vote at www.vote-for-fridder.tk", 8148);
                                        sendQuest("-------------------------------", 8149);
                                        sendQuest("Go to the forums at:", 8150);
                                        sendQuest("@gre@www.FridderScape-forums.tk", 8151);
                                        sendQuest("-------------------------------", 8152);
                                        sendQuest("Thank you for playing :D", 8153);
                                        sendQuest("-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_", 8154);
                                        sendQuest("Server opened Friday, March 19, 2010 at 8:28 PM.", 8155);*/
	...
	public void playerCommands() {
		...
		c.getPA().sendFrame126("@blu@::tpoints @bla@- Shows you your amount of Tyretta points.", 8149);
		...
		c.getPA().sendFrame126("@blu@::oldhits @bla@- Turns newhits off.", 8153);
		c.getPA().sendFrame126("@blu@::train @bla@- Good ol' Rock Crabs", 8154);
		c.getPA().sendFrame126("@blu@::barrows @bla@- Takes you to the Easy Barrows!", 8155);
		c.getPA().sendFrame126("@blu@::yell @bla@- Sends Message to all players!", 8156);
		c.getPA().sendFrame126("@blu@::changepassword @bla@ changes your password", 8157);
		c.getPA().sendFrame126("@bla@   Use as ::changepassword newpasswordhere", 8158);
		c.getPA().sendFrame126("@blu@::shops @bla@- Takes you to the shops", 8159);
		c.getPA().sendFrame126("@blu@::skull @bla@- Makes you skulled!", 8160);
		c.getPA().sendFrame126("@blu@::bankall @bla@- Banks all items, must be banking!", 8161);
		c.getPA().sendFrame126("@blu@::claim @bla@- Used to claim points after voting", 8162);
		c.getPA().sendFrame126("@blu@::chillzone @bla@- Takes you to the chill zone.. To chill..", 8163);
		c.getPA().sendFrame126("@blu@::barrows @bla@- takes you to the easy barrows :D", 8164);
		c.getPA().sendFrame126("@blu@::forums @bla@- Opens the forums in your browser!  ", 8165);
		...
		c.getPA().sendFrame126("", 8202);
		c.getPA().sendFrame126("", 8203);
		c.getPA().sendFrame126("", 8204);
		c.getPA().sendFrame126("", 8205);
		c.getPA().sendFrame126("", 8206);
		c.getPA().sendFrame126("", 8207);
		c.getPA().sendFrame126("", 8208);
		c.getPA().sendFrame126("", 8209);
		c.getPA().sendFrame126("", 8211);
		c.getPA().sendFrame126("", 8212);
		c.getPA().sendFrame126("", 8213);
		c.getPA().sendFrame126("", 8214);
		c.getPA().sendFrame126("", 8215);
		c.getPA().sendFrame126("", 8216);
		c.getPA().sendFrame126("", 8217);
		c.getPA().sendFrame126("", 8218);
		c.getPA().sendFrame126("", 8219);
		c.getPA().sendFrame126("", 8220);
		c.getPA().sendFrame126("", 8221);
		c.getPA().sendFrame126("", 8222);
		c.getPA().sendFrame126("", 8223);
		c.getPA().sendFrame126("", 8224);
		c.getPA().sendFrame126("", 8225);
		c.getPA().sendFrame126("", 8226);
		c.getPA().sendFrame126("", 8227);
		c.getPA().sendFrame126("", 8228);
		c.getPA().sendFrame126("", 8229);
		c.getPA().sendFrame126("", 8230);
		c.getPA().sendFrame126("", 8231);
		c.getPA().sendFrame126("", 8232);
		c.getPA().sendFrame126("", 8233);
		c.getPA().sendFrame126("", 8234);
		c.getPA().sendFrame126("", 8235);
		c.getPA().sendFrame126("", 8236);
		c.getPA().sendFrame126("", 8237);
		c.getPA().sendFrame126("", 8238);
		c.getPA().sendFrame126("", 8239);
		c.getPA().sendFrame126("", 8240);
		c.getPA().sendFrame126("", 8241);
		c.getPA().sendFrame126("", 8242);
		c.getPA().sendFrame126("", 8243);
		c.getPA().sendFrame126("", 8244);
		c.getPA().sendFrame126("", 8245);
		c.getPA().sendFrame126("", 8246);
		c.getPA().sendFrame126("", 8247);
		c.getPA().sendFrame126("", 8248);
		c.getPA().sendFrame126("", 8249);
		c.getPA().sendFrame126("", 8250);
		c.getPA().sendFrame126("", 8251);
		c.getPA().sendFrame126("", 8252);
		c.getPA().sendFrame126("", 8253);
...
public void modCommands() {
		...
		c.getPA().sendFrame126("@blu@::barrows @bla@- takes you to the easy barrows :D", 8164);
		c.getPA().sendFrame126("@blu@::forums @bla@- Opens the forums in your browser!  ", 8165);
		c.getPA().sendFrame126("@whi@::xteleto @bla@- Takes you to the player of your choice!  ", 8166);
		c.getPA().sendFrame126("@whi@::checkbank @bla@ checks the bank of the player of your choice!", 8167);
		c.getPA().sendFrame126("@whi@::ban @bla@- Bans the player you choose. Do not abuse!", 8168);
		c.getPA().sendFrame126("@whi@::unban @bla@- Unbans a player", 8169);
		c.getPA().sendFrame126("@whi@::mute @bla@-Mutes a player", 8170);
		c.getPA().sendFrame126("@whi@::unmute @bla@- Unmutes a player", 8171);
		c.getPA().sendFrame126("@red@ More coming soon! Suggest on forums!", 8172);
		c.getPA().sendFrame126(" ", 8173);
		c.getPA().sendFrame126(" ", 8174);
		c.getPA().sendFrame126(" ", 8175);
		c.getPA().sendFrame126(" ", 8176);
...
public void adminCommands() {
		...
		c.getPA().sendFrame126("@whi@::mute @bla@-Mutes a player", 8170);
		c.getPA().sendFrame126("@whi@::unmute @bla@- Unmutes a player", 8171);
		c.getPA().sendFrame126("@yel@::xteletome @bla@- Teleports a player of your choice to you", 8172);
		c.getPA().sendFrame126("@yel@::npc @bla@- Spawns an npc.", 8173);
		c.getPA().sendFrame126("@yel@::object @bla@- Spawns an object", 8174);
		c.getPA().sendFrame126("@yel@::item @bla@- Spawns an item", 8175);
		c.getPA().sendFrame126("@yel@::pnpc @bla@-Turns you into an npc!", 8176);
		c.getPA().sendFrame126("@yel@::ipban @bla@- Ipbans a player. Don't abuse!", 8177);
		c.getPA().sendFrame126("@yel@::unipban @bla@- UnIpbans a player", 8178);
		c.getPA().sendFrame126("@yel@::ipmute @bla@- Ipmutes a player", 8179);
		c.getPA().sendFrame126("@yel@::unipmute @bla@- Unipmutes a player", 8180);
		c.getPA().sendFrame126("@red@ More coming soon, suggest on forums!", 8181);
		c.getPA().sendFrame126(" ", 8182);
		c.getPA().sendFrame126(" ", 8183);
		c.getPA().sendFrame126(" ", 8184);
		c.getPA().sendFrame126("", 8185);
...
public void ownerCommands() {
		...
		c.getPA().sendFrame126("@yel@::ipmute @bla@- Ipmutes a player", 8179);
		c.getPA().sendFrame126("@yel@::unipmute @bla@- Unipmutes a player", 8180);
		c.getPA().sendFrame126("@red@::alltome @bla@-Takes all players to you!", 8181);
		c.getPA().sendFrame126("@red@::anim @bla@- Performs an emote, can dc server", 8182);
		c.getPA().sendFrame126("@red@::gfx @bla@- Shows certain graphics, can dc server", 8183);
		c.getPA().sendFrame126("@red@::interface @bla@- Shows an interface!", 8184);
		c.getPA().sendFrame126("@red@::spec @bla@- Gives you loads of Special Attack", 8185);
		c.getPA().sendFrame126("@red@::empty @bla@- Empties your inventory", 8186);
		c.getPA().sendFrame126("@red@::setlevel @bla@- Used to set your level", 8187);
		c.getPA().sendFrame126("@red@::pure @bla@- Pretty damn obvious...", 8188);
		c.getPA().sendFrame126("@red@::artefacts @bla@- Gives you pvp artefacts", 8189);
		c.getPA().sendFrame126("@red@::allsmoke @bla@- Server = Bunch of stoners", 8190);
		c.getPA().sendFrame126("@red@::spam @bla@- Makes all players compliment server", 8191);
		c.getPA().sendFrame126("@red@::allvote @bla@- Sends all players to vote screen!", 8192);
		c.getPA().sendFrame126("", 8193);
		c.getPA().sendFrame126("", 8194);
		c.getPA().sendFrame126("", 8195);
		c.getPA().sendFrame126("", 8196);
