...
public class AttackPlayer implements PacketType {
	...
	public void processPacket(Client c, int packetType, int packetSize) {
		...
		if(c.trade11 > 0) {
			...
			if (c.trade11 > 59) {
			c.sendMessage("@red@You can not pk scince you are new so please wait another "+mins+" minutes.");
			} else if (c.trade11 < 60) {
			c.sendMessage("@red@You can not pk scince you are new so please wait another "+c.trade11+" seconds.");
			}
		c.getCombat().resetPlayerAttack();
		c.getPA().movePlayer(3087, 3494, 0);
		return;
		}
			if (c.autocastId > 0)
				c.autocasting = true;
