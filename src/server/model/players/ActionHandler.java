...
c.sendMessage("You need a Barrow's Key to do this. Go to easy barrows");
}
break;
		case 11844:
			if (c.absX == 2936) {
		...
		break;
		case 4875:
         		c.getThieving().stealFromStall(995, 15000, 40, 25);
		break;
		case 4876:
         		c.getThieving().stealFromStall(995, 30000, 80, 50);
		break;
		case 4877:
         		c.getThieving().stealFromStall(995, 50000, 120, 75);
		break;
		case 4878:
         		c.getThieving().stealFromStall(995, 100000, 200, 90);
		break;
		case 4558:
		case 4559:
		...
		case 410:
			if (c.playerMagicBook < 2) {
				...
				c.sendMessage("You feel something very powerful rushing through your veins...");
				c.getPA().resetAutocast();
			} else {
				c.setSidebarInterface(6, 1151); //modern
				c.playerMagicBook = 0;
		...
		case 9356:
			c.getPA().enterCaves();
			c.sendMessage("Good luck. The fight caves are 10 waves. @red@RELOG FOR YOUR WAVES TO START.");
		break;
		case 1733:
        ...
        case 2472: //donor portal
	if (c.memberStatus == 1) {
	c.getPA().movePlayer(2718,9821, 0);
	c.sendMessage("You end up in a mysterious cave known as the Donator Zone.");
	} else {
		c.sendMessage("Hey! No cheating! You're not a donator!");
		c.sendMessage("To go to the donator zone buy donator status [$blu@Costs $5@bla@]");
		c.sendMessage("Talk to Damo or another staff member!");
	}
            break;
...
break;
			...
			break;
			case 6165:
				c.getThieving().stealFromStall(995, 10000, 40, 25);
			break;
			case 6166:
				c.getThieving().stealFromStall(995, 15000, 80, 50);
			break;
			case 6164:
				c.getThieving().stealFromStall(995, 20000, 120, 75);
			break;
			case 6162:
				c.getThieving().stealFromStall(995, 25000, 150, 90);
			break;
		case 4874:
         		c.getThieving().stealFromStall(995, 5000, 10, 1);
		break;
	...
	public void firstClickNpc(int npcType) {
		...
		switch(npcType) {
			...
			break;
			case 537:
				c.sendMessage("Welcome to the vote shop! You have " +c.votingPoints+ " Voting points!");
				c.getShops().openShop(14);
				c.sendMessage("If any items show up as 'Dwarf Remains' update your client from forums");
			break;
			case 883:
