	...
	**/
	...
	public void requestTrade(int id){
		...
		if(c.trade11 > 0) {
			...
			if (c.trade11 > 59) {
			c.sendMessage("@red@You can not trade scince you are new so please wait another "+mins+" minutes.");
			} else if (c.trade11 < 60) {
			c.sendMessage("@red@You can not trade scince you are new so please wait another "+c.trade11+" seconds.");
			}
		return;
		}
		if (c.connectedFrom.equals(o.connectedFrom)) {
			c.sendMessage("You cannot trade to your own IP.");
			return;
		}		
		if(c.playerRights == 2) {
