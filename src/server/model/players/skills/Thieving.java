...
public class Thieving {
	...
	public void stealFromNPC(int id) {
		if (System.currentTimeMillis() - c.lastThieve < 2000)
			return;
		if (Misc.random(50) == 27) {
			c.getPA().startTeleport(3084, 3494, 4, "ancient");
			c.sendMessage("@mag@To prevent autoclicking, you have been taken to a parallel world.");
			c.sendMessage("@mag@To get back, teleport home, or just chill here like a loner...");
			return;
		}
			...
			if (npcThieving[j][0] == id) {
				if (c.playerLevel[c.playerThieving] >= npcThieving[j][1]) {
					if (Misc.random(c.playerLevel[c.playerThieving] + 2 - npcThieving[j][1]) != 1) {
						...
						c.sendMessage("You attempt to pickpocket the man...");
						c.sendMessage("You recieve some gold.");
						break;
					} else {
