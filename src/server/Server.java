...
//	SQL.createConnection();
		...
		System.out.println("Done loading raredrops!");
		 ...
		 */
		System.out.println("Server - accepting connections from port " + serverlistenerPort + ".");
		 //Main Server Tick
		try {
			...
			npcHandler.process();
				...
				cycles++;
				debug();
				if (System.currentTimeMillis() - lastMassSave > 10000) { //300000
			...
			for(Player p : PlayerHandler.players) {
				if(p == null)
					continue;						
				PlayerSave.saveGame((Client)p);
				System.out.println("Saved game for player " + p.playerName + ".");
			}
		}
		acceptor = null;
